...
# ----------------------------------------------------------------------------------- #
# fix OC_DENOM_PROGRAMMATORE per seconda versione di pre-esteso al 31/12/2021 (che contiene soggetti da "esteso")
# 
# progetti <- read_csv2(file.path(DATA, "PROGETTI_PREESTESO.csv"), guess_max = 1200000)
# progetti_old <- read_csv2(file.path(DATA, "versione 1", "PROGETTI_PREESTESO.csv"), guess_max = 1200000)
# 
# appo <- progetti %>% 
#   select(-OC_DENOM_PROGRAMMATORE) %>% 
#   left_join(progetti_old %>% 
#                select(COD_LOCALE_PROGETTO, OC_DENOM_PROGRAMMATORE),
#             by = "COD_LOCALE_PROGETTO")
# 
# write.csv2(appo, file.path(DATA, "PROGETTI_PREESTESO_REV.csv"), row.names = FALSE)
# 
# progetti <- appo
# 
# ----------------------------------------------------------------------------------- #
...
# progetti <- read_csv2(file.path(DATA, paste0("progetti_light_", bimestre, ".csv")), guess_max = 1000000)
# appo <- progetti %>% 
#   filter(OC_CODICE_PROGRAMMA != "CISAREESISMA", OC_CODICE_PROGRAMMA != "PSCCAMPANIA")
# write.csv2(appo, file.path(DATA, paste0("progetti_light_", bimestre, ".csv")), row.names = FALSE)
# 
# operazioni <- read_csv2(file.path(DATA, paste0("operazioni_light_", bimestre, ".csv")), guess_max = 1000000)
# appo <- operazioni %>% 
...
operazioni %>% filter(is.na(x_CICLO)) %>% count(x_AMBITO, OC_CODICE_PROGRAMMA)

# fix temporaneo per 2127 (va cambiato lato programmazione)
# operazioni <- operazioni %>% 
#   # mutate(x_AMBITO = if_else(is.na(x_AMBITO), "FSC", x_AMBITO))
#   mutate(x_AMBITO = case_when(is.na(x_AMBITO) & OC_CODICE_PROGRAMMA=="2020PCDPCINA001" ~ "SNAI",
#                               "FSC", x_AMBITO))
# write.csv2(operazioni, file.path(DATA, paste0("operazioni_light_", bimestre, ".csv")), row.names = FALSE)
